...
class RGBPairDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        frame_i    = self.transform(Image.open(path_i).convert("RGB"))
        frame_next = self.transform(Image.open(path_next).convert("RGB"))
        return frame_i, frame_next, action

    def all_actions(self) -> torch.Tensor:
        """(Npairs, 6) — fast path for compute_action_stats."""
        return torch.stack([a for _, _, a in self._pairs])
...
def train(
    ...
    log_every:           int              = 1000,
    ckpt_path:           str | Path | None = "inverse_dynamics_conv.pt",
    ...
    for epoch in range(num_epochs):
        ...
        if val < best_val - 1e-5:
            ...
            if patience >= early_stop_patience:
                ...
                if use_wandb:
                    wandb.log({"early_stop_epoch": epoch, "best_val_loss": best_val})
                break
...
if __name__ == "__main__":
    ...
    train(model, train_loader, val_loader,
          num_epochs=args.num_epochs, device=args.device, use_wandb=args.wandb)
